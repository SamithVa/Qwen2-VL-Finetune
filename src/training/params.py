...
class ModelArguments:
    ...
    # ui graph
    uigraph_train: bool = field(default=True)  # Enable ui graph during training
    uigraph_test: bool = field(default=True)  # Enable ui graph during inference
    uigraph_diff: int = field(
        default=1
    )  # Pixel difference used for constructing ui grap
    uigraph_rand: bool = field(default=False)  # Enable random graph construction
    uimask_pre: bool = field(
        default=True
    )  # Prebuild patch selection mask in the preprocessor (not in model layers) for efficiency
    uimask_ratio: float = field(default=0.5)
    uimask_rand: bool = field(default=False)

    lm_skip_ratio: float = field(default=0.5)
    lm_skip_layer: str = field(default="[1,28,1]")
